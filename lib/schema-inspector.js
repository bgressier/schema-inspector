...
(function () {
	...
	_async.series = function (arr, callback) {
		callback = callback || function () {};
		if (!arr.length) {
			return callback();
		}
		...
		var iterate = function () {
			arr[completed](function (err, res) {
				if (err) {
					callback(err);
					callback = function () {};
				}
...
// Validation ------------------------------------------------------------------
	...
	var _asyncValidationAttribut = {
		asyncProperties: function (schema, candidate, callback) {
			...
			_async.series([
				function (next) {
					...
					_async.forEachSeries(Object.keys(candidate), function (i, done) {
						if (i in properties) {
							return done();
						}
						self._deeperObject(i);
						self._asyncValidate(properties['*'], candidate[i], function (err, res) {
							self._back();
							done(err, res);
						});
					}, next);
				...
				function (next) {
					_async.forEachSeries(Object.keys(properties), function (i, done) {
						if (i === '*') {
							return done();
						}
						self._deeperObject(i);
						self._asyncValidate(properties[i], candidate[i], function (err, res) {
							self._back();
							done(err, res);
						});
					}, next);
		...
		asyncItems: function (schema, candidate, callback) {
			...
			if (Array.isArray(items) && Array.isArray(candidate)) {
				_async.timesSeries(items.length, function (i, done) {
					self._deeperArray(i);
				...
				_async.timesSeries(candidate.length, function (i, done) {
					self._deeperArray(i);
					self._asyncValidate(items, candidate[i], function (err, res) {
						self._back();
						done(err, res);
					});
				}, callback);
	...
	Validation.prototype._asyncValidate = function (schema, candidate, callback) {
		...
		_async.series([
			function (next) {
				_async.forEachSeries(Object.keys(_validationAttribut), function (i, done) {
