...
(function () {
	...
	var _nextTick = (typeof process === 'object' && typeof process.nextTick === 'function')
		? process.nextTick
		: function (fn) {
	...
	Validation.prototype.validate = function (candidate, callback) {
		if (typeof callback === 'function') {
			...
			return _nextTick(function () {
				callback(null, self._validate(self._schema, candidate).result());
			});
		}
...
// Exports ---------------------------------------------------------------------
	...
	SchemaInspector.sanitize = function (schema, post, custom) {
		return new Sanitization(schema, custom).sanitize(post);
	};
