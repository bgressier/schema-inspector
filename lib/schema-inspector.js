...
(function () {
	...
	_async.forEach = function (arr, iterator, callback) {
		...
		arr.forEach(function (elem) {
			iterator(elem, function (err, res) {
				if (err) {
					callback(err);
					callback = function () {};
				}
	...
	_async.timesSeries = function (n, iterator, callback) {
		callback = callback || function () {};
		if (!n) {
			return callback();
		}
		var completed = 0;
		var iterate = function () {
			iterator(completed, function (err) {
				if (err) {
					callback(err);
					callback = function () {};
				}
	...
	_async.nextTick = (typeof process === 'object' && typeof process.nextTick === 'function')
		? process.nextTick
		: function (fn) {
...
// Validation ------------------------------------------------------------------
	var _validationAttribut = {
		...
		exec: function (schema, candidate, callback) {
			...
			if (typeof callback === 'function') {
				return _async.forEachSeries(Array.isArray(schema.exec) ? schema.exec : [schema.exec], function (exec, done) {
					if (typeof exec !== 'function') {
						return done();
					}
					exec.call(self, schema, candidate, done);
				}, callback);
			}
		...
		properties: function (schema, candidate, callback) {
			...
			//
			if (typeof callback === 'function') {
				return _async.forEachSeries(Object.keys(properties), function (i, done) {
					if (i === '*') {
						return done();
					}
					self._deeperObject(i);
					self._validate(properties[i], candidate[i], function (err, res) {
						self._back();
						done(err, res);
					});
				}, callback);
			}
			// else if Synchronous call
			//
		...
		items: function (schema, candidate, callback) {
			...
			if (Array.isArray(items) && Array.isArray(candidate)) {
				if (typeof callback === 'function') {
					return _async.timesSeries(items.length, function (i, done) {
						self._deeperArray(i);
						self._validate(items[i], candidate[i], function (err, res) {
							self._back();
							done(err, res);
						});
						self._back();
				...
				if (typeof callback === 'function') {
					return _async.timesSeries(candidate.length, function (i, done) {
						self._deeperArray(i);
						self._validate(items, candidate[i], function (err, res) {
							self._back();
							done(err, res);
						});
					}, callback);
				}
				for (i = 0, l = candidate.length; i < l; i++) {
					this._deeperArray(i);
	...
	function Validation(schema, custom) {
		Inspection.prototype.constructor.call(this, schema, custom);
		var _error = [];
	...
	Validation.prototype.validate = function (candidate, callback) {
		if (typeof callback === 'function') {
			var self = this;
			return _async.nextTick(function () {
				self._validate(self._schema, candidate, function (err) {
	...
	// TODO: Handle async for this._custom
	Validation.prototype._validate = function (schema, candidate, callback) {
		...
		if (typeof callback === 'function') {
			return _async.forEachSeries(Object.keys(_validationAttribut), function (i, done) {
				...
				_async.forEachSeries(Object.keys(self._custom), function (i, done) {
...
// Sanitization ----------------------------------------------------------------
	...
	Sanitization.prototype.sanitize = function (post, callback) {
		if (typeof callback === 'function') {
			var self = this;
			return _async.nextTick(function () {
				self._sanitize(self._schema, post);
