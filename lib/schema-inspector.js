...
(function () {
	...
	// Async extension -----------------------------------------------------------
	var async = {};
	_extend(async, root.async);

	async.timesSeries = function (n, iterator, callback) {
	...
	var _validationAttribut = {
		...
		exec: function (schema, candidate, callback) {
			...
			if (typeof callback === 'function') {
				return async.forEachSeries(Array.isArray(schema.exec) ? schema.exec : [schema.exec], function (exec, done) {
	...
	var _asyncValidationAttribut = {
		...
		asyncItems: function (schema, candidate, callback) {
			...
			if (Array.isArray(items) && Array.isArray(candidate)) {
				...
				async.timesSeries(candidate.length, function (i, done) {
					self._deeperArray(i);
...
// Sanitization ----------------------------------------------------------------
	...
	var _asyncCanitizationAttribut = {
		...
		asyncItems: function (schema, post, callback) {
			...
			if (Array.isArray(items) && Array.isArray(post)) {
				...
				async.timesSeries(post.length, function (i, next) {
					self._deeperArray(i);
					self._asyncSanitize(items, post[i], function (err, res) {
						if (err) {
							return next();
						}
						post[i] = res;
	...
	_extend(Sanitization.prototype, _asyncCanitizationAttribut);
